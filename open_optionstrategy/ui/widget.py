...
from ..base import APP_NAME, EVENT_OPTION_LOG, EVENT_OPTION_STRATEGY, STRATEGY_LABELS
from ..engine import OptionStrategyEngine


class OptionStrategyManager(QtWidgets.QWidget):
    """期权策略管理面板"""
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine) -> None:
        ...
        self.strategy_engine: OptionStrategyEngine = main_engine.get_engine(APP_NAME)
    ...
    def register_event(self) -> None:
        ...
        self.event_engine.register(
            EVENT_OPTION_STRATEGY, self.signal_strategy.emit
        )
    ...
    def add_strategy(self) -> None:
        ...
        labels: Mapping[str, str] = self.strategy_engine.classes[class_name].labels
        editor: SettingEditor = SettingEditor(parameters, class_name=class_name, labels=labels)
        n: int = editor.exec_()

        if n == editor.DialogCode.Accepted:
            setting: dict = editor.get_setting()
            gateway_name: str = setting.pop("gateway_name")
            strategy_name: str = setting.pop("strategy_name")

            self.strategy_engine.add_strategy(
                class_name, strategy_name, gateway_name, setting
            )
...
class StrategyManager(QtWidgets.QFrame):
    ...
    def __init__(
        self,
        strategy_manager: OptionStrategyManager,
        strategy_engine: OptionStrategyEngine,
        data: dict
    ) -> None:
        """构造函数"""
        super().__init__()

        self.strategy_manager: OptionStrategyManager = strategy_manager
        self.strategy_engine: OptionStrategyEngine = strategy_engine
    ...
    def init_ui(self) -> None:
        ...
        gateway_name: str = self._data["gateway_name"]
        class_name: str = self._data["class_name"]
        author: str = self._data["author"]

        label_text: str = (
            f"{strategy_name}  -  {gateway_name}  -  ({class_name} by {author})"
        )
        label: QtWidgets.QLabel = QtWidgets.QLabel(label_text)
        label.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)

        labels: Mapping[str, str] = self.strategy_engine.strategies[strategy_name].labels
        self.parameters_monitor: DataMonitor = DataMonitor(self._data["parameters"], labels)
        self.variables_monitor: DataMonitor = DataMonitor(self._data["variables"], labels)
    ...
    def edit_strategy(self) -> None:
        ...
        labels: Mapping[str, str] = self.strategy_engine.strategies[strategy_name].labels
        editor: SettingEditor = SettingEditor(parameters, strategy_name=strategy_name, labels=labels)
        n: int = editor.exec_()
...
class DataMonitor(QtWidgets.QTableWidget):
    ...
    def __init__(self, data: dict, labels: Mapping[str, str]) -> None:
        """构造函数"""
        super().__init__()

        self._data: dict = data
        self.labels: Mapping[str, str] = labels
        self.cells: dict = {}
    ...
    def init_ui(self) -> None:
        """初始化界面"""
        headers: list = [self.labels.get(name, name) for name in self._data]
...
class LogMonitor(BaseMonitor):
    ...
    event_type: str = EVENT_OPTION_LOG
    data_key: str = ""
    sorting: bool = False
...
class SettingEditor(QtWidgets.QDialog):
    ...
    def init_ui(self) -> None:
        ...
        if self.class_name:
            ...
            button_text: str = "添加"
            parameters: dict = {"strategy_name": "", "gateway_name": ""}
        ...
        for name, value in parameters.items():
            ...
            edit.setToolTip(f"{name} {type_.__name__}")
            form.addRow(self.labels.get(name, name), edit)
